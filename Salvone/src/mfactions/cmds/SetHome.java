...
public class SetHome {
	...
	@Command(name="faction.sethome", aliases={"fac.sethome", "f.sethome"}, permission="mfactions.sethome", inGameOnly=true)
	public void factionSubHomeSet(CommandArgs args) {
		...
		FactionManager.getInstance().getFaction(p.getName()).setHome(p.getLocation());
		
		FactionManager.getInstance().getFaction(p.getName()).messageFaction(rank.getColor() + p.getName() + " &3has set the faction home!");
